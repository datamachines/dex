...
$ wget https://raw.githubusercontent.com/datamachines/dex/${DEX_VERSION}/api/api.proto
...
__NOTE:__ Because dex has the `google.golang.org/grpc` package in its `vendor` directory, gRPC code in `github.com/datamachines/dex/api` refers to the vendored copy, not copies in a developers GOPATH. Clients must either regenerate the gRPC Go code or vendor dex and remove its `vendor` directory to run this program.
